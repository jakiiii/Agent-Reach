...
class TestCLI:
    ...
    def test_twitter_config_does_not_run_unsafe_verification_or_mutate_env(
        ...
        assert "`twitter status` is not executed" in output
    ...
    def test_install_reddit_deps_routes_by_environment(self, monkeypatch):
        """Desktop uses OpenCLI; server uses the pinned rdt-cli Git source."""
        calls = []
    ...
    def test_install_server_dry_run_skips_opencli_only_channels(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "require a desktop environment + Chrome and are skipped on servers: boss, facebook, instagram, opencli" in out
...
class TestVersionCompare:
    ...
    def test_local_ahead_of_release_no_downgrade_prompt(self):
        """A local main build ahead of the release must not prompt a downgrade."""
...
class TestWatchVersionCompare:
    def test_watch_does_not_prompt_downgrade(self, monkeypatch, capsys):
        """watch follows check-update semantics and must not prompt when local is ahead."""
        class R:
            status_code = 200
            headers = {}
